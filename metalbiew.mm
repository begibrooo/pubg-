...
static ImVec4 colorBone      = ImVec4(1.f, 1.f, 1.f, 1.f);
static ImVec4 colorRadarBg   = ImVec4(0.06f, 0.06f, 0.09f, 0.65f);
...
ImGui::Begin(SDKvaT(make_string("WE9MSVlBUk9WIFZJUCBAa2hvbDF5YXJvdnZ2")).c_str(), &MenDeal, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize);
...
if (Settings::Tab == 0) 
{
    ...
    ImGui::Checkbox("Alert 360", &imgui::Alert360);
    configManager::putBoolean(config, "ESP", "ALERT_360", imgui::Alert360);

    ImGui::Checkbox("Mini-Radar (2D)", &imgui::MiniRadar);
    configManager::putBoolean(config, "ESP", "MINI_RADAR", imgui::MiniRadar);

    ImGui::Checkbox("FOV Threat Arrow", &imgui::FOVArrow);
    configManager::putBoolean(config, "ESP", "FOV_ARROW", imgui::FOVArrow);

    ImGui::Checkbox("Smart FPS (Anti-Heat)", &imgui::SmartFPS);
    configManager::putBoolean(config, "ESP", "SMART_FPS", imgui::SmartFPS);
...
ImGui::PopStyleColor();
    ...
    ImGui::ColorEdit4("Visible Enemy Colour", (float*)&visibleColor, colFlags);
    ImGui::ColorEdit4("Hidden Enemy Colour", (float*)&unvisibleColor, colFlags);
    ImGui::ColorEdit4("Skeleton Bone Colour", (float*)&colorBone, colFlags);
    ...
    ImGui::ColorEdit4("Radar Background", (float*)&colorRadarBg, colFlags);
...
-(void) readIniConfig {
    ...
    imgui::MiniRadar   = configManager::readBoolean(config, "ESP", "MINI_RADAR", true);
    imgui::FOVArrow    = configManager::readBoolean(config, "ESP", "FOV_ARROW", true);
    imgui::SmartFPS    = configManager::readBoolean(config, "ESP", "SMART_FPS", true);
...
long obbbbl() { //GUObjectArray 
  ...
  if([BundID containsString:@"com.tencent.ig"]){
      return getRealOffset(0x10A7F93E0); //Обновлено на 4.5.0 (gl)
  }
  if([BundID containsString:@"kr"]){
      return getRealOffset(0x10A8ACA60); //Обновлено на 4.5.0 (kr)
  }
  if([BundID containsString:@"rekoo"]){
      return getRealOffset(0x10A888A60); //Обновлено на 4.5.0 (TW)
  }
  if([BundID containsString:@"vn.vng.pubgmobile"]){ //Обновлено на 4.5.0 (VNG)
...
TNameEntryArray *GetGNames() { //GnameFunction
   NSDictionary *infoDictionary = [[NSBundle mainBundle] infoDictionary];
      ...
      if([BundID containsString:@"ig"]){
          return ((TNameEntryArray *(*)()) ((unsigned long)Get_module_base() +0x105014128))(); //Обновлено на 4.5.0 (GL)
      }
      if([BundID containsString:@"kr"]){
          return ((TNameEntryArray *(*)()) ((unsigned long)Get_module_base()+0x10A671060))(); //Обновлено на 4.5.0 (KR)
      }
      if([BundID containsString:@"rekoo"]){
          return ((TNameEntryArray *(*)()) ((unsigned long)Get_module_base()+0x10A650080))(); //Обновлено на 4.5.0 (tw)
      }
      if([BundID containsString:@"vn"]){
          return ((TNameEntryArray *(*)()) ((unsigned long)Get_module_base()+0x104D8E0DC))(); //Обновлено на 4.5.0 (VNG)
      }
      if([BundID containsString:@"imobile"]){
...
bool bIsAimingAtLocalPlayer(ASTExtraPlayerCharacter* Player) {
    if (!Player || !g_LocalPlayer || !Player->RootComponent || !g_LocalPlayer->RootComponent)
        return false;
    ...
    float dot = DotProduct(Forward, NormalizeVector(Direction));
...
void DrawSkeletonAdvanced(ASTExtraPlayerCharacter* Player, bool isBot, bool isVisible) {
    ...
    if (isBot) {
        ...
        boneColor = isVisible ? ImColor(visibleColor) : ImColor(colorBone);
    }
...
{
    if (!g_LocalPlayer || !g_LocalPlayer->RootComponent) return;
    ...
    for (auto* p : enemies)
    {
        if (!p || !p->RootComponent || p->bDead) continue;
...
void DrawThreatFOVArrow(ASTExtraPlayerCharacter* p, ImDrawList* d)
{
    if (!p || !g_LocalPlayer || !g_LocalPlayer->RootComponent || p->bDead) return;
    if (!bIsAimingAtLocalPlayer(p)) return;

    ImGuiIO& io = ImGui::GetIO();
    ImVec2 screenCenter(io.DisplaySize.x * 0.5f, io.DisplaySize.y * 0.5f);

    FVector lpPos = g_LocalPlayer->RootComponent->K2_GetComponentLocation();
    FVector lpForward = g_LocalPlayer->GetActorForwardVector();
    lpForward.Z = 0;
    lpForward = NormalizeVector(lpForward);

    FVector enemyPos = p->RootComponent->K2_GetComponentLocation();
    FVector toEnemy = enemyPos - lpPos;
    toEnemy.Z = 0;
    toEnemy = NormalizeVector(toEnemy);

    float dot = lpForward.X * toEnemy.X + lpForward.Y * toEnemy.Y;
    float det = lpForward.X * toEnemy.Y - lpForward.Y * toEnemy.X;
    float angle = atan2f(det, dot);

    float arrowRadius = 140.0f;
    float arrowX = screenCenter.x + sinf(angle) * arrowRadius;
    float arrowY = screenCenter.y - cosf(angle) * arrowRadius;

    float t = ImGui::GetTime();
    ImU32 arrowColor = ((int)(t * 8.0f) & 1) ? IM_COL32(255, 30, 30, 245) : IM_COL32(255, 230, 0, 245);
    ...
    ImVec2 tip(arrowX + dirX * baseLen, arrowY + dirY * baseLen);
    ImVec2 left(arrowX - dirX * (baseLen * 0.35f) + normX * halfWidth, arrowY - dirY * (baseLen * 0.35f) + normY * halfWidth);
    ImVec2 right(arrowX - dirX * (baseLen * 0.35f) - normX * halfWidth, arrowY - dirY * (baseLen * 0.35f) - normY * halfWidth);

    d->AddTriangleFilled(tip, left, right, arrowColor);
    d->AddTriangle(tip, left, right, IM_COL32(0, 0, 0, 220), 1.5f);
}

void Draw2DMiniRadar(const std::vector<ASTExtraPlayerCharacter*>& enemies)
{
    if (!imgui::MiniRadar) return;
    ...
    float radius = 55.0f;
    ImVec2 center(io.DisplaySize.x - radius - 30.0f, radius + 85.0f);
    float maxDistMeters = 200.0f;

    ImU32 bgCol = ImGui::GetColorU32(colorRadarBg);
    d->AddCircleFilled(center, radius, bgCol, 36);
    d->AddCircle(center, radius, IM_COL32(255, 215, 0, 200), 36, 1.5f);

    d->AddCircle(center, radius * 0.5f, IM_COL32(255, 255, 255, 45), 24, 1.0f);
    d->AddCircle(center, radius * 0.25f, IM_COL32(255, 255, 255, 30), 24, 1.0f);

    d->AddLine(ImVec2(center.x - radius, center.y), ImVec2(center.x + radius, center.y), IM_COL32(255, 255, 255, 35), 1.0f);
    d->AddLine(ImVec2(center.x, center.y - radius), ImVec2(center.x, center.y + radius), IM_COL32(255, 255, 255, 35), 1.0f);

    d->AddTriangleFilled(
        ImVec2(center.x, center.y - 6.0f),
    ...
    FVector lpForward = lp->GetActorForwardVector();
    lpForward.Z = 0;
    lpForward = NormalizeVector(lpForward);
    ...
    for (auto* p : enemies)
    {
        ...
        if (imgui::HideBot && isBot) continue;

        FVector enemyPos = p->RootComponent->K2_GetComponentLocation();
        FVector toEnemy = enemyPos - lpPos;
        float distVal = sqrtf(toEnemy.X*toEnemy.X + toEnemy.Y*toEnemy.Y + toEnemy.Z*toEnemy.Z) / 100.0f;

        if (distVal < 0.5f || distVal > 400.0f) continue;

        FVector enemy2D = toEnemy;
        enemy2D.Z = 0;
        enemy2D = NormalizeVector(enemy2D);

        float dot = lpForward.X * enemy2D.X + lpForward.Y * enemy2D.Y;
        float det = lpForward.X * enemy2D.Y - lpForward.Y * enemy2D.X;
        float angle = atan2f(det, dot);

        float rDist = (distVal / maxDistMeters) * (radius - 5.0f);
        bool clamped = false;
        if (rDist > radius - 4.0f) {
            rDist = radius - 4.0f;
            clamped = true;
        }

        float dotX = center.x + sinf(angle) * rDist;
        float dotY = center.y - cosf(angle) * rDist;

        ImU32 dotColor;
        bool isAiming = bIsAimingAtLocalPlayer(p);

        if (isAiming) {
            float t = ImGui::GetTime();
            dotColor = ((int)(t * 6.0f) & 1) ? IM_COL32(255, 0, 0, 255) : IM_COL32(255, 255, 0, 255);
        } else if (isBot) {
            dotColor = IM_COL32(0, 200, 255, clamped ? 160 : 230);
        } else {
            dotColor = IM_COL32(255, 50, 50, clamped ? 180 : 255);
        }

        if (clamped) {
            ImVec2 tip(center.x + sinf(angle) * (radius - 2.0f), center.y - cosf(angle) * (radius - 2.0f));
            d->AddCircleFilled(tip, 2.5f, dotColor);
        } else {
            d->AddCircleFilled(ImVec2(dotX, dotY), isAiming ? 4.5f : 3.5f, dotColor);
            d->AddCircle(ImVec2(dotX, dotY), isAiming ? 5.5f : 4.0f, IM_COL32(0, 0, 0, 180), 12, 1.0f);
        }
    }

    if (mainFont) {
        const char* radarLabel = "RADAR 200m";
        d->AddText(mainFont, 9.0f, ImVec2(center.x - 22.0f, center.y + radius + 2.0f), IM_COL32(255, 255, 255, 180), radarLabel);
    }
}
...
// Проверка видимости (с оптимизацией Smart FPS против нагрева)
isVisible = false;
if (imgui::SmartFPS) {
    if (distVal <= 250.0f && (headOk || rootOk)) {
        FVector bonePos = p->GetBonePos("Head", {});
        if (bonePos.X != 0 || bonePos.Y != 0 || bonePos.Z != 0) {
            isVisible = g_PlayerController->LineOfSightTo(p, bonePos, true);
        }
        if (!isVisible && distVal <= 100.0f) {
            FVector pelvisPos = p->GetBonePos("pelvis", {});
            if (pelvisPos.X != 0 || pelvisPos.Y != 0 || pelvisPos.Z != 0) {
                isVisible = g_PlayerController->LineOfSightTo(p, pelvisPos, true);
            }
        }
    }
} else {
...
make_string("WE9MSVlBUk9WIFZJUA").c_str()];
